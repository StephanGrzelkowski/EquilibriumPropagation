...
arrTrainImages = np.random.permutation(len(train_set[0]))
    ...
    for n in xrange(setting.nTestImages):  # test for 10 images
        ...
        if setting.debugEql == 1:
            if np.argmax(arrOutputUnits) == arrLabels[arrTestImages[n]]:
                nCorrect += 1.0
            #print "Max acitivity unit (train Image set): ", np.argmax(arrOutputUnits), ". Correct label: ", arrLabels[
#                arrTestImages[n]], "; \n"
        elif np.argmax(arrOutputUnits) == test_set[1][arrTestImages[n]]:
            nCorrect += 1.0
            #print "Max acitivity unit: ", np.argmax(arrOutputUnits), ". Correct label: ", test_set[1][
#                arrTestImages[n]], "; \n"
